...
(function (global) {
    ...
    /* ---------- 色阶定义 ---------- */
    /* 3 级：用于生长模型评分（越高越亮）。
     * 【口径统一 2026-09-14】原为 5 级（断点 85/70/55/40，标签「优秀/良好/中等/偏低/较差」），
     * 与屏上其它位置的「风险三档」（<70 高风险 / 70–79 预警 / ≥80 正常）断点不一致，
     * 于是同一块屏会出现「70 分：图例说良好、乡镇综合评价表说预警」两套说法。
     * 现断点与标签全部对齐风险三档。
     *
     * 【为什么用 gt/gte/lt/lte 而不是 min/max】piecewise 的 min/max 是闭区间，
     * 相邻档共用端点时会重叠，而重叠分段取「后匹配者」——
     * 实测 70 分会被 max=70 的那档抢走、80 分被 max=80 的那档抢走，
     * 于是「70 分」在图上是高风险配色、在表格/tooltip 里却是预警。
     * 改用半开区间 [80,100] / [70,80) / [0,70) 后端点归属唯一，两边不再打架。
     * 第三档保留 gte:0 下界是有意为之：傅坊乡用 -1 做无数据哨兵，
     * 必须落到 outOfRange（灰色斜线），不能被「< 70」吃掉。
     *
     * 配色仍取 RAMP 的中亮段（4/3/2）：最低档刻意不用 RAMP[0]，
     * 因为最深的 #0C2A55 在深色面板上几乎不可见，而 <70 恰恰是最需要被看见的短板乡镇
     * （与 barColor 的同类取舍一致）。
     * 影响面：本函数仅生长模型评分使用，病虫害屏走 riskPieces()，不受影响。 */
    function scorePieces() {
        return [
            { gte: 80, lte: 100, label: '≥ 80　正常', color: RAMP[4] },
            { gte: 70, lt: 80,   label: '70 – 79　预警', color: RAMP[3] },
            { gte: 0,  lt: 70,   label: '< 70　高风险', color: RAMP[2] }
        ];
    }
